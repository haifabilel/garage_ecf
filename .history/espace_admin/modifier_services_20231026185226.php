...
if(isset($_POST['addService'])){
    ...
    header('location:modifier_services.php');
    $data = [
        ':titre' => $titre,
        ':description' => $desc,
        ':image' => $img_des,
...
<tbody>
    ...
    <?php
     ...
     while($user = $req->fetch()){
        ?>
        <tr>
        <td><img src="<?=$user['image']?>" alt="image_card" style="height:150px; width:100%;" ></td>
        <td><?=$user['titre']?></td>
        <td><?=$user['description']?></td>
        <td>
        <a class="btn btn-primary" href="supprimer_user.php?id=<?=$user['id']?>">Modifier <i class="fa-regular fa-pen-to-square" style="color: #ffffff;"></i></a><br><br>
