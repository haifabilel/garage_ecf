...
if(isset($_POST['Update'])){
  ...
  if(isset($titre) && isset($description) && isset($image)){
    ...
    $req =$conn->query("UPDATE services SET titre = '$titre' , description = '$description', image = '$image_des' WHERE id = ? ");
    if($req){
      header('location:fetch_service.php');
    }
...
<div class="modal fade" id="ServiceUpdate" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
  ...
  <form action="" method="POST" enctype="multipart/form-data">
    <div class="modal-content">
      ...
      <div class="form-group">
    <textarea class="form-control" name="description"  value="<?php echo $row['description']; ?>" placeholder="Description..."required></textarea><br>
  </div>
  <div class="form-group mb-3">
