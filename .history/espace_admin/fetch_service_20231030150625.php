...
<tbody>
    ...
    <?php
     ...
     while($user = $req->fetch()){
        ?>
        <tr>
        <td><img src="<?=$user['image']?>" alt="image_card" style="height:150px; width:250px;" ></td>
        <td><?=$user['titre']?></td>
        <td><?=$user['description']?></td>
        <td>
        <a  class="btn btn-primary"href="id=<?=$user['id']?>" data-bs-toggle="modal" data-bs-target="#ServiceUpdate"><i class="fa-regular fa-pen-to-square" style="color: #ffffff;"></i></a><br><br>
...
<div class="modal fade" id="ServiceUpdate" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
  <form action="" method="POST" enctype="multipart/form-data">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title fs-5" id="exampleModalToggleLabel">Upadte service</h3>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
      <div class="form-group">
      <input type="hidden" name="id"">
       <input type="text" name="titre"  value="<?php echo $user['titre']; ?>" class="form-control"  placeholder="Titre de service" required><br>
      </div>
      <div class="form-group">
    <textarea class="form-control" name="description"  value="<?php echo $user['description']; ?>" placeholder="Description..."required></textarea><br>
  </div>
  <div class="form-group mb-3">
  <input type="file" name="image"  value="<?php echo $user['image']; ?>" class="form-control">
</div>
