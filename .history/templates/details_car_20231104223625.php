...
function getVoituresById(PDO $conn, int $id):array|bool
{
    $query = $conn->prepare('SELECT * FROM poll WHERE id = :id');
...
function getPollResultsByPollId(PDO $conn, int $id): array
{
    $query = $conn->prepare('SELECT * FROM poll_item WHERE poll_id = :id');
    $query->bindValue(':id', $id, PDO::PARAM_INT);
    $query->execute();
